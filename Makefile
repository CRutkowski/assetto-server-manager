...
install-linter:
	which golangci-lint || curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/master/install.sh | sh -s -- -b $$(go env GOPATH)/bin v1.63.4
...
vet: install-linter generate
	go mod tidy
	go vet ./...
	golangci-lint run --enable bodyclose,misspell,gofmt,unconvert,goimports --timeout 30m --exclude-files content_cars_skins.go,plugin_kissmyrank_config.go,plugin_realpenalty_config.go
